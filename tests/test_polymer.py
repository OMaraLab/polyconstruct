...
def test_complex_polymer(data_dir: Path, output_dir: Path):    
    ...
    for i in range(12):
        if last_monomer == "arginine":
            ...
            polymer.extend(arg_monomer, from_junction_name= 'N', to_junction_name= 'C')
            last_monomer = "arginine"
    
    # extend the backbone in both directions with 2 more arginines
    for i in range(2):
        polymer.extend(arg_monomer, from_junction_name= 'N', to_junction_name= 'C')
        polymer.extend(arg_monomer, from_junction_name= 'C', to_junction_name= 'N')
    ...
    while polymer.has_junction("N-side-chain") and countdown > 0:
        if random.random() < 0.2:
            polymer.extend(arg_monomer, from_junction_name= 'N-side-chain', to_junction_name= 'C')
        else:
            polymer.extend(glu_monomer, from_junction_name= 'N-side-chain', to_junction_name= 'C')            
        countdown -= 1
